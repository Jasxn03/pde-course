...
x_basis = de.Fourier(coords['x'], Nx, bounds = (0,Lx), dealias=3/2, dtype = np.complex128)
y_basis = de.Fourier(coords['y'], Ny, bounds = (0, Ly), dealias=3/2, dtype = np.complex128)
...
# uniform initial condition with some random fluctuations
...
while solver.proceed and t < t_end:
    ...
    mass1 = np.mean(abs(psi_1['g'])**2)
    mass2 = np.mean(abs(psi_2['g'])**2)
    print(f"t={t:.3f}, max|psi1|={np.max(abs(psi_1['g'])):.3e}, "
         f"max|psi2|={np.max(abs(psi_2['g'])):.3e}, "
         f"<|psi1|^2>={mass1:.6e}, <|psi2|^2>={mass2:.6e}")
