"""
From Dedalus - https://github.com/DedalusProject/dedalus/blob/master/examples/ivp_2d_rayleigh_benard/plot_snapshots.py
...
def main(filename, start, count, output):
    ...
    def imag_func(xmesh, ymesh, data):
        ...
        print(data.shape)
        print(data[0, 0])
        return xmesh, ymesh, np.imag(data)
    ...
    with h5py.File(filename, mode='r') as file:
        for index in range(start, start+count):
            for n, task in enumerate(tasks):
                ...
                plot_tools.plot_bot_3d(dset, 0, index, axes=axes, title=task, even_scale=True, visible_axes=False, func=abs_func) # choose what to plot!! 
            # Add time title
